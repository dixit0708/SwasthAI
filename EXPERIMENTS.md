...
To prevent the Pneumonia CNN (ResNet18) from providing confident misdiagnoses on non-X-ray images, we employ a three-signal ensemble OOD detection approach.
...
To solve this, we combine two distinct heuristics. An image is rejected if **any** heuristic flags it as OOD:
...
3. **Face Detection (cv2.FaceDetectorYN)**
   - **How it works:** A lightweight ONNX DNN face detection model built into OpenCV 5. It scans the original grayscale image for recognizable human facial features.
   - **Why?** Some complex indoor photography (e.g., portrait photos with muted colors) can have a low average saturation while coincidently containing gradient patterns that trick the Mahalanobis check. Since valid chest X-rays never contain a face, detecting a face is a robust, deterministic trigger to immediately reject the image as OOD.
